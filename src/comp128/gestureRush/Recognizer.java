...
public class Recognizer {
    ...
    private final List<GestureTemplate> templateArray = new ArrayList<>();
    ...
    Deque<Point> copyArray = new ArrayDeque<>(points);
    Deque<Point> normalizedArray = gestureNormalise(copyArray);
    templateArray.add(new Template(name, normalizedArray));
}
    public Result recognize(Deque<Point> points){
        if (points == null || points.size() < 2 || templateArray.isEmpty()){
            return new Result("no result",0.0);
        }
        Deque<Point> potentialTemplate = this.gestureNormalise(new ArrayDeque<>(points));
        double maxDistance = Double.MAX_VALUE;
        String closestName = null;

        for (Template x :templateArray){
            double y = distanceAtBestAngle(potentialTemplate, x.points);
            if (y < maxDistance){
                maxDistance = y;
                closestName = x.gestureName;
            }
        }
        double finalScore = 1 - (maxDistance / TEMPLATE_DIAGONAL);
        if (finalScore < 0){
            finalScore = 0;
        }
        if (finalScore >= 1){
        ...
        return new Result(closestName, finalScore);
    }
